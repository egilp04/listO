...
const GestionItem: React.FC<GestionItemProps> = ({ item: propItem }) => {
  ...
  const manejarEnvio = async () => {
    ...
    if (exito) {
      setNotificacion(
        mEdicion
          ? t("gestionItem.notifModificado")
          : t("gestionItem.notifAñadido"),
        "exito",
      );
      navegar("/biblioteca");
    } else {
  ...
  return (
    <form
      className="form-registro mx-auto my-4 lg:my-6 md:w-11/12! lg:w-200! min-h-fit! p-4! md:p-6! lg:p-8! gap-4! md:gap-5! lg:gap-6!"
      onSubmit={(e) => {
        e.preventDefault();
        manejarEnvio();
      }}
    >
      <header className="w-full mb-1 lg:mb-2">
        <h2 className="text-center font-bold text-primary-1100 dark:text-primary-50 text-xl md:text-2xl lg:text-3xl">
          {mEdicion
            ? t("gestionItem.tituloEditar")
            : t("gestionItem.tituloCrear")}
        </h2>
      </header>
      ...
      <div className="grid-registro gap-3! md:gap-4! border-none p-0 m-0 w-full items-center">
        ...
        <section className="flex flex-col gap-1 w-full">
          <span className="font-medium text-primary-900 dark:text-primary-50 mb-1">
            {t("gestionItem.campoValoracion")}
          </span>
      ...
      <fieldset className="w-full bg-white dark:bg-primary-900 rounded-xl overflow-hidden shadow-sm border border-neutral-200 dark:border-primary-800">
        ...
        {generoAbierto && (
          <div className="p-2 md:p-3 flex flex-col gap-2 bg-neutral-50 dark:bg-primary-950 min-h-24 justify-center">
            {!tipoSeleccionado ? (
              <div className="flex flex-col items-center justify-center text-primary-500 dark:text-primary-300 gap-1 py-4 text-center">
                <span className="material-symbols-outlined text-4xl mb-1 opacity-80">
                  lock
                </span>
                <p className="font-medium">
                  Selecciona primero un tipo para desbloquear los géneros
                </p>
              </div>
            ) : (
              generos
                ...
                .map((genero) => (
                  ...
                  >
                    <div className="bg-primary-950 dark:bg-primary-700 text-white px-4 py-1.5 rounded-md font-medium min-w-personalizado-150 text-center shadow-sm">
                      {genero.nombre}
                    </div>
                    <Checkbox
                      ...
                      manejarCambio={() =>
                        manejarCambioGenero(genero.id_genero)
                      }
                    />
                  </div>
                ))
            ...
            {tipoSeleccionado &&
              generos.filter((g) => g.id_tipo === tipoSeleccionado).length ===
                0 && (
      ...
      <footer className="mt-2 lg:mt-4 flex justify-center w-full">
        ...
        >
          {guardando
            ? t("gestionItem.botonGuardando")
            : mEdicion
              ? t("gestionItem.botonModificar")
              : t("gestionItem.botonAñadir")}
        </Button>
      </footer>
    </form>
  );
