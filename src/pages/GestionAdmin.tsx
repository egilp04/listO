...
const GestionAdmin = () => {
  ...
  return (
    ...
    >
      ...
      <article className="shadow-elevation-3 bg-primary-50 dark:bg-primary-975 flex flex-col gap-6 max-h-150 overflow-hidden pt-10 pb-10 rounded-sm">
        <div className="flex flex-col gap-4 justify-center items-center md:justify-around md:flex-row w-full">
          <div className="w-1/2">
            <label htmlFor="buscador-admin" className="sr-only">
              {t("gestionAdmin.filtrarLabel", {
                tipo:
                  selected === "usuario"
                    ? t("gestionAdmin.botonUsuarios").toLowerCase()
                    : t("gestionAdmin.botonGeneros").toLowerCase(),
              })}
            </label>
            <input
              id="buscador-admin"
              className="input-style-comun input-border-primario dark:border-primary-50 input-responsive dark:text-primary-50"
              placeholder={t("gestionAdmin.buscadorPlaceholder", {
                tipo: selected,
              })}
          ...
          <div className="flex items-center">
            <Button variant="primario" onClick={crearItem} name={selected}>
              <span>
                {selected === "usuario"
                  ? t("gestionAdmin.crearUsuario")
                  : t("gestionAdmin.crearGenero")}
              </span>
            </Button>
          </div>
        </div>
